    ...
    if (id && extend) {
        data.read('tokens', id, (err, tokenData) => {
            if (!err && tokenData) {
                ...
                if (tokenObject.expires > Date.now()) {
                    ...
                    data.update('tokens', id, tokenObject, (err) => {
                        if (!err) {
                            ...
                            callback(500, {error: 'there was a servber side error'})
                        }
                    })
                } else {
                ...
                callback(500, {error: 'There was a server side error to fetch the toke...'})
            }
        })
    } else {
            ...
            if (!err && tokenData) {
                data.delete('tokens', id, (err) => {
                    if (!err) {
                        ...
                        callback(500, {error: 'Server side error in deleting the token...'})
                    }
                })
            } else {
...
handler._token.verify = (id, phone, callback) => {
    data.read('tokens', id, (err, tokenData) => {
        if (!err && tokenData) {
            if (parseJSON(tokenData).id === id && parseJSON(tokenData).expires > Date.now()) {
                callback(true)
            } else {
                callback(false)
            }
